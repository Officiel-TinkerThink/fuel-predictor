...
| `similar_operations` | Past operations, ranked: same vehicle → same vehicle type → same vehicle group → same category; then same activity mode; then nearest distance and lifting hours. Each row says where it came from (`source`: `dataset` = imported prepared-fuel history with `operation_date` and `source_reference`; `recorded` = an operation planned through the app/agent with `stop_sequence`, the estimate given then, and `actual_fuel_liters` once recorded), and a `match` block (`vehicle`: `same`/`same_type`/`same_group`/`same_category` — `same_type` only appears once the owner has split a group into types, `activity_mode`, `distance_delta_km`, `lifting_hours_delta`, `score`). |
